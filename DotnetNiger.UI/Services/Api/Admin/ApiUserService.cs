...
public class ApiUserService : ApiServiceBase, IUserService
{
    ...
    {
        var response = await Http.GetAsync(ApiEndpoints.Admin.Users);
    ...
    {
        var response = await Http.GetAsync($"{ApiEndpoints.Admin.Users}/{userId}");
        if (!response.IsSuccessStatusCode)
            return null;
    ...
    public async Task<UserDto?> CreateUserAsync(CreateUserRequest user)
    {
        ...
        var response = await Http.PostAsync(ApiEndpoints.Admin.Users, content);
        if (!response.IsSuccessStatusCode)
            return null;
    ...
    public async Task<UserDto?> UpdateUserAsync(UserDto user)
    {
        ...
        if (statusChanged)
        {
            ...
            var statusResponse = await Http.PatchAsync($"{ApiEndpoints.Admin.Users}/{user.Id}/status", statusContent);
        ...
        if (teamChanged)
        {
            ...
            var teamResponse = await Http.PatchAsync($"{ApiEndpoints.Admin.Users}/{user.Id}/team", teamContent);
        ...
        if (rolesChanged)
        {
            ...
            foreach (var role in rolesToRemove)
            {
                var response = await Http.DeleteAsync(
                    string.Format(ApiEndpoints.Admin.UserRole, user.Id, role));
                if (!response.IsSuccessStatusCode)
                    return null;
            }
            foreach (var role in rolesToAdd)
            {
                ...
                var response = await Http.PostAsync(
                    string.Format(ApiEndpoints.Admin.UserRoles, user.Id), content);
                if (!response.IsSuccessStatusCode)
                    return null;
            }
        }
    ...
    {
        var response = await Http.DeleteAsync($"{ApiEndpoints.Admin.Users}/{userId}");
    ...
    public async Task<bool> ApproveUserAsync(Guid userId)
    {
        ...
        var response = await Http.PatchAsync($"{ApiEndpoints.Admin.Users}/{userId}/status", statusContent);
        if (!response.IsSuccessStatusCode)
            return false;

        var roleContent = JsonContent.Create(new UpdateUserRolesRequest { RoleName = "Collaborator" });
        await Http.PostAsync(string.Format(ApiEndpoints.Admin.UserRoles, userId), roleContent);
    ...
    public async Task<bool> RejectUserAsync(Guid userId)
    {
        ...
        var response = await Http.PatchAsync($"{ApiEndpoints.Admin.Users}/{userId}/status", content);
        return response.IsSuccessStatusCode;
    }
    ...
    public async Task<bool> AssignRoleAsync(Guid userId, string roleName)
    {
        ...
        var response = await Http.PostAsync(
            string.Format(ApiEndpoints.Admin.UserRoles, userId), content);
        return response.IsSuccessStatusCode;
    }
    ...
    {
        var response = await Http.DeleteAsync(
            string.Format(ApiEndpoints.Admin.UserRole, userId, roleName));
        return response.IsSuccessStatusCode;
    }
