...
public class ApiEventService : ApiServiceBase, IEventService
{
    ...
    {
        var response = await Http.GetAsync($"{ApiEndpoints.Events}/upcoming");
    ...
    {
        var response = await Http.GetAsync($"{ApiEndpoints.Events}?status=pending");
        if (!response.IsSuccessStatusCode)
            return new List<EventDto>();
    ...
    {
        var endpoint = $"{ApiEndpoints.Events}/{eventId}/reject?reason={Uri.EscapeDataString(reason)}";
        var response = await Http.PatchAsync(endpoint, null);
        return response.IsSuccessStatusCode;
    }
