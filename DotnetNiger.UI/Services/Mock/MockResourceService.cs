...
public class MockResourceService : IResourceService
{
    ...
    public async Task<ResourceDto?> UpdateResourceAsync(Guid id, UpdateResourceRequest request)
    {
        var resource = _resources.FirstOrDefault(r => r.Id == id);
        if (resource is null) return await Task.FromResult<ResourceDto?>(null);

        if (request.Title is not null) resource.Title = request.Title;
        if (request.Slug is not null) resource.Slug = request.Slug;
        if (request.Description is not null) resource.Description = request.Description;
        if (request.Url is not null) resource.Url = request.Url;
        // DownloadUrl and ThumbnailUrl not present in ResourceDto mock
        if (request.ResourceType is not null) resource.ResourceType = request.ResourceType;
        if (request.Level is not null) resource.Level = request.Level;
