...
public class MockEventService : IEventService
{
    ...
    public async Task<EventDto?> UpdateEventAsync(Guid id, UpdateEventRequest request)
    {
        ...
        if (request.Slug is not null) ev.Slug = request.Slug;
        ...
        if (request.EventType is not null) ev.EventType = request.EventType;
        ...
        if (request.Capacity.HasValue) ev.Capacity = request.Capacity.Value;
        if (request.MeetupLink is not null) ev.MeetupLink = request.MeetupLink;
