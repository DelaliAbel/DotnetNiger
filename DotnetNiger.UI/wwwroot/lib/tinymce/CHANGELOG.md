...
- The `tox-button` and `tox-button-primary` buttons now support the `hover`, `active`, `focus`, and `disabled` states. #TINY-9713
...
- New `buttonType` property on dialog button components, supporting `toolbar` style in addition to `primary` and `primary`. #TINY-8304
...
- Added font related Oxide variables for primary buttons, allowing for custom styling. #TINY-6061
