...
/// <summary>Configure le pipeline middleware de l'application Identity.</summary>
public static class PipelineSetup
{
    /// <summary>Configure le pipeline HTTP (middleware, sécurité, santé).</summary>
    public static WebApplication ConfigureApp(WebApplicationBuilder builder)
    {
        ...
        {
            options.SwaggerEndpoint("/swagger/v1/swagger.json", "DotnetNiger Identity API v1");
        ...
        app.MapGet("/health/downstream", async ([FromServices] IdentityDbContext idCtx) =>
        {
            try
            {
                ...
                return Results.Ok(new
                {
                    ...
                    service = "DotnetNiger.Identity",
                    timestamp = DateTime.UtcNow,
                    checks = new { database = "connected", downstream = "not_checked" }
                });
            }
            catch (Exception ex)
            {
