...
public class GdprExportService
{
    ...
    public GdprExportService(IdentityDbContext db, UserManager<ApplicationUser> userManager)
    {
        _db = db;
    ...
    public async Task<(byte[] ZipData, string UserName)> ExportUserDataAsync(Guid userId)
    {
        ...
        var tenants = await _db.Tenants
            .Where(t => t.Id == user.TenantId).ToListAsync();
        ...
        using (var archive = new ZipArchive(memoryStream, ZipArchiveMode.Create, true))
        {
            await AddJsonToArchive(archive, "profile.json", new
            {
                ...
                user.LastName, user.AvatarUrl, user.TenantId,
                user.IsActive, user.EmailConfirmed, user.TwoFactorEnabled,
                user.CreatedAt
            });
            ...
            if (tenants.Count != 0)
                await AddJsonToArchive(archive, "tenants.json", tenants);
        }
