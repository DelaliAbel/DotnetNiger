...
/// <summary>Seed des événements Community avec médias, inscriptions, intervenants et commentaires.</summary>
static class SeedCommunityEventService
{
    static readonly Guid AdminId = Guid.Parse("A1B2C3D4-E5F6-7890-ABCD-EF1234567890");

    /// <summary>Seed 10 événements avec médias, intervenants, inscriptions et tags.</summary>
    ...
    static List<Event> SeedEventData(AppDbContext db, DateTime now)
    {
        var events = new List<Event>
        {
            new() { Id = Guid.NewGuid(), Title = "Meetup .NET 9 : Les nouveautés", Slug = "meetup-dotnet9-nouveautes", Description = "Venez découvrir les nouveautés de .NET 9 : performances, nouvelles API et évolutions du langage C#. Au programme : démos en direct, retours d'expérience et Q&A.", Location = "Niamey, Niger", EventType = "meetup", Category = "Développement Web", StartDate = now.AddDays(-20), EndDate = now.AddDays(-20).AddHours(3), CreatedBy = AdminId, OrganizerName = "Admin Plateforme", Capacity = 50, RegisteredCount = 42, IsPublished = true, PublishedAt = now.AddDays(-30), CreatedAt = now.AddDays(-30), UpdatedAt = now.AddDays(-20), CoverImageUrl = "https://images.unsplash.com/photo-1540575467063-178a50c2df87?w=800" },
            new() { Id = Guid.NewGuid(), Title = "Workshop Blazor Avancé", Slug = "workshop-blazor-avance", Description = "Workshop pratique de 4h : composants avancés, State management (Fluxor), SignalR temps réel et déploiement sur Azure Static Web Apps.", Location = "En ligne", EventType = "workshop", Category = "Développement Web", StartDate = now.AddDays(-10), EndDate = now.AddDays(-10).AddHours(4), CreatedBy = AdminId, OrganizerName = "Admin Plateforme", Capacity = 30, RegisteredCount = 28, IsPublished = true, PublishedAt = now.AddDays(-25), CreatedAt = now.AddDays(-25), UpdatedAt = now.AddDays(-10), CoverImageUrl = "https://images.unsplash.com/photo-1516321318423-f06f85e504b3?w=800", MeetupLink = "https://meet.google.com/abc-defg-hij" },
            new() { Id = Guid.NewGuid(), Title = "Conférence IA Générative avec .NET", Slug = "conference-ia-generative-dotnet", Description = "Conférence complète sur l'intégration de l'IA générative dans vos apps .NET : OpenAI, Azure OpenAI, Semantic Kernel, ML.NET et bonnes pratiques de prompt engineering.", Location = "Niamey, Niger", EventType = "conference", Category = "Data & IA", StartDate = now.AddDays(5), EndDate = now.AddDays(5).AddHours(6), CreatedBy = AdminId, OrganizerName = "Admin Plateforme", Capacity = 100, RegisteredCount = 58, IsPublished = true, PublishedAt = now.AddDays(-15), CreatedAt = now.AddDays(-15), UpdatedAt = now, CoverImageUrl = "https://images.unsplash.com/photo-1485827404703-89b55fcc595e?w=800" },
            new() { Id = Guid.NewGuid(), Title = "Hackathon Smart City Niger", Slug = "hackathon-smart-city-niger", Description = "48h pour créer des solutions innovantes pour les villes intelligentes au Niger. Thèmes : mobilité, énergie, eau, santé et éducation. Lots à gagner pour les 3 meilleures équipes.", Location = "Niamey, Niger", EventType = "hackathon", Category = "Communauté", StartDate = now.AddDays(15), EndDate = now.AddDays(17), CreatedBy = AdminId, OrganizerName = "Admin Plateforme", Capacity = 60, RegisteredCount = 37, IsPublished = true, PublishedAt = now.AddDays(-10), CreatedAt = now.AddDays(-10), UpdatedAt = now, CoverImageUrl = "https://images.unsplash.com/photo-1504384308090-c894fdcc538d?w=800" },
            new() { Id = Guid.NewGuid(), Title = "Formation Docker & Kubernetes", Slug = "formation-docker-kubernetes", Description = "Formation intensive de 2 jours sur Docker et Kubernetes pour les développeurs .NET. TP pratiques : multi-stage builds, Docker Compose, déploiement sur AKS.", Location = "En ligne", EventType = "workshop", Category = "DevOps", StartDate = now.AddDays(25), EndDate = now.AddDays(26), CreatedBy = AdminId, OrganizerName = "Admin Plateforme", Capacity = 25, RegisteredCount = 22, IsPublished = true, PublishedAt = now.AddDays(-5), CreatedAt = now.AddDays(-5), UpdatedAt = now, CoverImageUrl = "https://images.unsplash.com/photo-1667372393119-3d4c48d07fc9?w=800" },
            new() { Id = Guid.NewGuid(), Title = "Webinaire Cybersécurité pour développeurs", Slug = "webinaire-cybersecurite-developpeurs", Description = "Bonnes pratiques de sécurité : OWASP Top 10, sécurisation des API REST, gestion des secrets avec Azure Key Vault, authentication JWT et refresh tokens.", Location = "En ligne", EventType = "webinar", Category = "Sécurité", StartDate = now.AddDays(40), EndDate = now.AddDays(40).AddHours(2), CreatedBy = AdminId, OrganizerName = "Admin Plateforme", Capacity = 100, RegisteredCount = 19, IsPublished = true, PublishedAt = now.AddDays(-1), CreatedAt = now.AddDays(-1), UpdatedAt = now, CoverImageUrl = "https://images.unsplash.com/photo-1555949963-ff9fe0c870eb?w=800" },
            new() { Id = Guid.NewGuid(), Title = "Meetup Débutant C# : Les bases solides", Slug = "meetup-debutant-csharp-bases", Description = "Soirée dédiée aux débutants en C# : variables, types, classes, héritage, interfaces, LINQ et async/await. Apportez votre ordinateur pour coder avec nous !", Location = "Niamey, Niger", EventType = "meetup", Category = "Développement Web", StartDate = now.AddDays(50), EndDate = now.AddDays(50).AddHours(2), CreatedBy = AdminId, OrganizerName = "Admin Plateforme", Capacity = 40, RegisteredCount = 15, IsPublished = true, PublishedAt = now.AddDays(0), CreatedAt = now, UpdatedAt = now, CoverImageUrl = "https://images.unsplash.com/photo-1531482615713-2afd69097998?w=800" },
            new() { Id = Guid.NewGuid(), Title = "Conférence Cloud Azure pour .NET", Slug = "conference-cloud-azure-dotnet", Description = "Découvrez comment tirer parti d'Azure pour vos applications .NET : App Services, Functions, Cosmos DB, Service Bus et monitoring avec Application Insights.", Location = "Niamey, Niger", EventType = "conference", Category = "Cloud", StartDate = now.AddDays(60), EndDate = now.AddDays(60).AddHours(5), CreatedBy = AdminId, OrganizerName = "Admin Plateforme", Capacity = 80, RegisteredCount = 31, IsPublished = true, PublishedAt = now.AddDays(1), CreatedAt = now, UpdatedAt = now, CoverImageUrl = "https://images.unsplash.com/photo-1560264280-88b68371db39?w=800" },
            new() { Id = Guid.NewGuid(), Title = "Workshop API RESTful avec .NET 9", Slug = "workshop-api-restful-dotnet9", Description = "Atelier pratique pour construire une API RESTful complète avec .NET 9 : Minimal APIs, validation FluentValidation, versioning, Swagger/OpenAPI et tests d'intégration.", Location = "En ligne", EventType = "workshop", Category = "Développement Web", StartDate = now.AddDays(75), EndDate = now.AddDays(75).AddHours(3), CreatedBy = AdminId, OrganizerName = "Admin Plateforme", Capacity = 35, RegisteredCount = 12, IsPublished = true, PublishedAt = now.AddDays(2), CreatedAt = now, UpdatedAt = now, CoverImageUrl = "https://images.unsplash.com/photo-1558494949-ef010cbdcc31?w=800" },
            new() { Id = Guid.NewGuid(), Title = "Hackathon Open Source Weekend", Slug = "hackathon-open-source-weekend", Description = "Un week-end pour contribuer à des projets open source .NET. Encadrement par des mainteneurs, découverte de Git et GitHub, et soumission de vos premières PR.", Location = "En ligne", EventType = "hackathon", Category = "Open Source", StartDate = now.AddDays(90), EndDate = now.AddDays(92), CreatedBy = AdminId, OrganizerName = "Admin Plateforme", Capacity = 50, RegisteredCount = 8, IsPublished = true, PublishedAt = now.AddDays(3), CreatedAt = now, UpdatedAt = now, CoverImageUrl = "https://images.unsplash.com/photo-1526925539333-8b91bafd4b02?w=800" },
        };
        db.Events.AddRange(events);
        return events;
    }

    static void SeedEventMedia(AppDbContext db, List<Event> events)
    {
        var ev = (int i) => events[i];
        db.Set<EventMedia>().AddRange(
            new EventMedia { Id = Guid.NewGuid(), EventId = ev(2).Id, Type = "image", Url = "https://images.unsplash.com/photo-1485827404703-89b55fcc595e?w=800", Title = "Conférence IA - Photo 1" },
            new EventMedia { Id = Guid.NewGuid(), EventId = ev(2).Id, Type = "image", Url = "https://images.unsplash.com/photo-1489875347897-49f64b51c1f8?w=800", Title = "Conférence IA - Photo 2" },
            new EventMedia { Id = Guid.NewGuid(), EventId = ev(3).Id, Type = "image", Url = "https://images.unsplash.com/photo-1504384308090-c894fdcc538d?w=800", Title = "Hackathon Smart City" },
            new EventMedia { Id = Guid.NewGuid(), EventId = ev(7).Id, Type = "image", Url = "https://images.unsplash.com/photo-1560264280-88b68371db39?w=800", Title = "Conférence Azure" }
        );
    }

    static void SeedEventSpeakers(AppDbContext db, List<Event> events)
    {
        var ev = (int i) => events[i];
    ...
    static void SeedEventRegistrations(AppDbContext db, DateTime now, List<Event> events)
    {
        var ev = (int i) => events[i];
    ...
    static void SeedEventTags(AppDbContext db, List<Event> events, List<Tag> tags)
    {
        var t = (string n) => tags.First(x => x.Name == n);
        var ev = (int i) => events[i];
    ...
    static void SeedEventComments(AppDbContext db, DateTime now, List<Event> events)
    {
        var ev = (int i) => events[i];
        db.Comments.AddRange(
            new Comment { Id = Guid.NewGuid(), Content = "Super conférence ! Le passage sur Semantic Kernel était très intéressant.", UserId = AdminId, AuthorName = "Admin Plateforme", AuthorAvatar = "https://ui-avatars.com/api/?name=Admin+Plateforme&background=512BD4&color=fff", EventId = ev(2).Id, CreatedAt = now.AddDays(-3) },
            new Comment { Id = Guid.NewGuid(), Content = "Hâte de participer au hackathon ! J'ai déjà une idée de projet.", UserId = AdminId, AuthorName = "Admin Plateforme", AuthorAvatar = "https://ui-avatars.com/api/?name=Admin+Plateforme&background=512BD4&color=fff", EventId = ev(3).Id, CreatedAt = now.AddDays(-2) },
            new Comment { Id = Guid.NewGuid(), Content = "Est-ce que la formation sera disponible en replay ?", UserId = AdminId, AuthorName = "Admin Plateforme", AuthorAvatar = "https://ui-avatars.com/api/?name=Admin+Plateforme&background=512BD4&color=fff", EventId = ev(4).Id, CreatedAt = now.AddDays(-1) },
            new Comment { Id = Guid.NewGuid(), Content = "Le meetup débutant C# tombe au bon moment pour mon équipe !", UserId = AdminId, AuthorName = "Admin Plateforme", AuthorAvatar = "https://ui-avatars.com/api/?name=Admin+Plateforme&background=512BD4&color=fff", EventId = ev(6).Id, CreatedAt = now }
        );
    }
}
