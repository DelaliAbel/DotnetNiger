...
/// <summary>Seed des articles, commentaires et tags associés dans Community.</summary>
static class SeedCommunityContentService
{
    static readonly Guid AdminId = Guid.Parse("A1B2C3D4-E5F6-7890-ABCD-EF1234567890");

    /// <summary>Seed les articles, commentaires et leurs relations.</summary>
    public static List<Post> Seed(AppDbContext db, DateTime now, List<Tag> tags, List<Category> cats)
    {
    ...
    static List<Post> SeedPostData(AppDbContext db, DateTime now)
    {
    ...
    static void SeedPostRelations(AppDbContext db, List<Post> posts, List<Tag> tags, List<Category> cats)
    {
    ...
    static void SeedCommentData(AppDbContext db, DateTime now, List<Post> posts)
    {
        var p = (int i) => posts[i];
        var c1 = Guid.NewGuid(); var c2 = Guid.NewGuid(); var c3 = Guid.NewGuid();
        db.Comments.AddRange(
            new Comment { Id = c1, Content = "Excellent article ! J'ai pu créer ma première app Blazor. Merci !", UserId = AdminId, AuthorName = "Admin Plateforme", AuthorAvatar = "https://ui-avatars.com/api/?name=Admin+Plateforme&background=512BD4&color=fff", PostId = p(0).Id, CreatedAt = now.AddDays(-59) },
            new Comment { Id = c2, Content = "Très bon guide. Est-ce que vous pourriez approfondir sur l'intégration avec SignalR ?", UserId = AdminId, AuthorName = "Admin Plateforme", AuthorAvatar = "https://ui-avatars.com/api/?name=Admin+Plateforme&background=512BD4&color=fff", PostId = p(0).Id, CreatedAt = now.AddDays(-57) },
            new Comment { Id = c3, Content = "Bien sûr ! Je prévois un article dédié à SignalR + Blazor la semaine prochaine.", UserId = AdminId, AuthorName = "Admin Plateforme", AuthorAvatar = "https://ui-avatars.com/api/?name=Admin+Plateforme&background=512BD4&color=fff", PostId = p(0).Id, ParentCommentId = c2, CreatedAt = now.AddDays(-56) },
            new Comment { Id = Guid.NewGuid(), Content = "L'architecture microservices est vraiment adaptée pour notre projet. Merci pour les conseils sur Ocelot.", UserId = AdminId, AuthorName = "Admin Plateforme", AuthorAvatar = "https://ui-avatars.com/api/?name=Admin+Plateforme&background=512BD4&color=fff", PostId = p(2).Id, CreatedAt = now.AddDays(-29) },
            new Comment { Id = Guid.NewGuid(), Content = "Docker a changé ma façon de développer. Excellent article introductif !", UserId = AdminId, AuthorName = "Admin Plateforme", AuthorAvatar = "https://ui-avatars.com/api/?name=Admin+Plateforme&background=512BD4&color=fff", PostId = p(4).Id, CreatedAt = now.AddDays(-19) },
            new Comment { Id = Guid.NewGuid(), Content = "La sécurité est souvent négligée. Merci de rappeler les bonnes pratiques.", UserId = AdminId, AuthorName = "Admin Plateforme", AuthorAvatar = "https://ui-avatars.com/api/?name=Admin+Plateforme&background=512BD4&color=fff", PostId = p(5).Id, CreatedAt = now.AddDays(-11) },
            new Comment { Id = Guid.NewGuid(), Content = "Les problèmes N+1 m'ont déjà fait perdre des heures. Article très utile !", UserId = AdminId, AuthorName = "Admin Plateforme", AuthorAvatar = "https://ui-avatars.com/api/?name=Admin+Plateforme&background=512BD4&color=fff", PostId = p(6).Id, CreatedAt = now.AddDays(-9) },
            new Comment { Id = Guid.NewGuid(), Content = "GitHub Actions est vraiment puissant. Merci pour ce guide pas à pas.", UserId = AdminId, AuthorName = "Admin Plateforme", AuthorAvatar = "https://ui-avatars.com/api/?name=Admin+Plateforme&background=512BD4&color=fff", PostId = p(7).Id, CreatedAt = now.AddDays(-6) },
            new Comment { Id = Guid.NewGuid(), Content = "SignalR c'est génial pour les notifications en temps réel. Je l'utilise pour mon dashboard.", UserId = AdminId, AuthorName = "Admin Plateforme", AuthorAvatar = "https://ui-avatars.com/api/?name=Admin+Plateforme&background=512BD4&color=fff", PostId = p(8).Id, CreatedAt = now.AddDays(-4) },
            new Comment { Id = Guid.NewGuid(), Content = "Kubernetes en production c'est un vrai game changer. Bel article !", UserId = AdminId, AuthorName = "Admin Plateforme", AuthorAvatar = "https://ui-avatars.com/api/?name=Admin+Plateforme&background=512BD4&color=fff", PostId = p(9).Id, CreatedAt = now.AddDays(-2) },
            new Comment { Id = Guid.NewGuid(), Content = "Content que ça t'ait plu. Un article sur Kubernetes + Azure va suivre.", UserId = AdminId, AuthorName = "Admin Plateforme", AuthorAvatar = "https://ui-avatars.com/api/?name=Admin+Plateforme&background=512BD4&color=fff", PostId = p(9).Id, ParentCommentId = null, CreatedAt = now.AddDays(-1) }
        );
    }
}
