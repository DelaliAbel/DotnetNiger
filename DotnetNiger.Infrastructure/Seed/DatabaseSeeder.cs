...
public static class DatabaseSeeder
{
    public static async Task SeedAsync(IServiceProvider services)
    {
        ...
        await SeedIdentityService.SeedAsync(provider);
        await SeedCommunityService.SeedAsync(provider);
        await ClientSetupService.SetupAsync(provider);
    }
}

public static class ClientSetupService
{
    public static async Task SetupAsync(IServiceProvider services)
    {
        ...
        await CreateTestClientAsync(db, appManager);
    ...
    static async Task CreateTestClientAsync(DotnetNigerDbContext db, IOpenIddictApplicationManager appManager)
    {
        ...
        db.OAuthClients.Add(new OAuthClient
        {
            ...
            ApplicationId = appId!,
            ClientId = "test-client",
            ClientSecretHash = HashSHA256("test-secret"),
            ClientName = "Test Client",
            Description = "Client de test pour les tests OAuth2",
            RedirectUris = "[]",
            PostLogoutRedirectUris = "[]",
            AllowedGrantTypes = System.Text.Json.JsonSerializer.Serialize(new[] { "password", "refresh_token", "client_credentials" }),
            IsActive = true,
        });
