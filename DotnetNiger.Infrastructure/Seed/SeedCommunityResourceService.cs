...
public static class SeedCommunityResourceService
{
    ...
    public static void SeedProjectData(DotnetNigerDbContext db, DateTime now)
    {
        db.Projects.AddRange(
            new Project { Id = Guid.NewGuid(), Title = "Plateforme E-commerce DotnetNiger", Slug = "plateforme-e-commerce-dotnetniger", Description = "Plateforme e-commerce complète avec Blazor WebAssembly et .NET 9, intégration paiements Mobile Money (Orange Money, Wave), panier, wishlist et dashboard admin.", ImageUrl = "https://images.unsplash.com/photo-1556742049-0cfed4f6a45d?w=800", Technologies = "Blazor,.NET 9,Entity Framework,SQL Server,SignalR", Status = "active", CreatedBy = AdminId, AuthorName = "Admin Plateforme", IsFeatured = true, IsPublished = true, CreatedAt = now.AddDays(-90) },
            new Project { Id = Guid.NewGuid(), Title = "API Gateway DotnetNiger", Slug = "api-gateway-dotnetniger", Description = "Passerelle API centralisée avec Ocelot : routage dynamique, authentification JWT, rate limiting, circuit breaker Polly et monitoring Prometheus/Grafana.", ImageUrl = "https://images.unsplash.com/photo-1558494949-ef010cbdcc31?w=800", Technologies = "Ocelot,.NET 9,Polly,Prometheus,Swagger", Status = "active", CreatedBy = AdminId, AuthorName = "Admin Plateforme", IsFeatured = true, IsPublished = true, CreatedAt = now.AddDays(-80) },
            new Project { Id = Guid.NewGuid(), Title = "Application Mobile Meetup", Slug = "application-mobile-meetup", Description = "App .NET MAUI pour les meetups DotnetNiger avec notifications push, agenda, speaker bios, QR code check-in et sondages en direct via SignalR.", ImageUrl = "https://images.unsplash.com/photo-1512941937669-90a1b58e7e9c?w=800", Technologies = ".NET MAUI,C#,SignalR,Azure Push", Status = "active", CreatedBy = AdminId, AuthorName = "Admin Plateforme", IsFeatured = false, IsPublished = true, CreatedAt = now.AddDays(-70) },
            new Project { Id = Guid.NewGuid(), Title = "Plateforme de Mentorat", Slug = "plateforme-de-mentorat", Description = "Plateforme connectant mentors et mentorés dans le domaine tech au Niger : matching intelligent, planning de sessions, suivi de progression et badges de certification.", ImageUrl = "https://images.unsplash.com/photo-1524178232363-1fb2b075b655?w=800", Technologies = "Blazor,.NET 9,SignalR,Cosmos DB", Status = "active", CreatedBy = AdminId, AuthorName = "Admin Plateforme", IsFeatured = true, IsPublished = true, CreatedAt = now.AddDays(-60) },
            new Project { Id = Guid.NewGuid(), Title = "CLI DotnetNiger", Slug = "cli-dotnetniger", Description = "Outil en ligne de commande pour la communauté : scaffold de projets, génération de code, vérification des bonnes pratiques et publication d'articles via terminal.", ImageUrl = "https://images.unsplash.com/photo-1629654297299-c8506221ca97?w=800", Technologies = "C#,System.CommandLine,.NET 9", Status = "beta", CreatedBy = AdminId, AuthorName = "Admin Plateforme", IsFeatured = false, IsPublished = true, CreatedAt = now.AddDays(-30) }
        );
    }
    ...
    static void SeedNewsletters(DotnetNigerDbContext db, DateTime now)
    {
        // No initial newsletters
    }

    static void SeedSiteSettings(DotnetNigerDbContext db)
    {
        db.SiteSettings.AddRange(
            ...
            new SiteSetting { Id = "site.maintenance", Key = "site.maintenance", Value = "false", Type = "boolean", Description = "Mode maintenance", UpdatedAt = DateTime.UtcNow },
            new SiteSetting { Id = "site.slogan", Key = "site.slogan", Value = "Ensemble, construisons l'avenir numérique du Niger", Type = "string", Description = "Slogan du site", UpdatedAt = DateTime.UtcNow },
            new SiteSetting { Id = "site.email.contact", Key = "site.email.contact", Value = "contact@dotnetniger.com", Type = "string", Description = "Email de contact", UpdatedAt = DateTime.UtcNow }
        );
    }
}