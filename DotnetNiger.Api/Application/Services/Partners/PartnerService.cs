...
public class PartnerService : IPartnerService
{
    ...
    public async Task<List<PartnerResponse>> GetAllActiveAsync(string? partnerType, CancellationToken ct = default)
    {
        ...
        var partners = await q.OrderBy(p => p.SortOrder).ThenBy(p => p.Name).ToListAsync(ct);
        return partners.Select(MapToResponse).ToList();
    }

    /// <summary>Récupère un partenaire par identifiant.</summary>
    public async Task<PartnerResponse?> GetByIdAsync(Guid id, CancellationToken ct = default)
    {
        var p = await _db.Set<Partner>().FindAsync(id, ct);
        return p == null ? null : MapToResponse(p);
    }

    /// <summary>Crée un nouveau partenaire.</summary>
    public async Task<PartnerResponse> CreateAsync(CreatePartnerRequest request, CancellationToken ct = default)
    {
        var partner = new Partner
        {
            Id = Guid.NewGuid(),
            Name = request.Name,
            Slug = await GenerateUniqueSlug(null, request.Name, ct),
    ...
    public async Task<PartnerResponse?> UpdateAsync(Guid id, UpdatePartnerRequest request, CancellationToken ct = default)
    {
        var partner = await _db.Set<Partner>().FindAsync(id, ct);
        if (partner == null) return null;
        partner.Name = request.Name;
        partner.Slug = await GenerateUniqueSlug(null, request.Name, ct);
    ...
    private async Task<string> GenerateUniqueSlug(string? providedSlug, string name, CancellationToken ct = default)
    {
        ...
        var suffix = 1;
        while (await _db.Set<Partner>().AnyAsync(p => p.Slug == candidate, ct))
        {
